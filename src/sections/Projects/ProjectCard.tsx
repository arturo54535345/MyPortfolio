...
export const ProjectCard = ({ title, description, image, technologies, link, github }: ProjectCardProps) => {
  ...
  return (
    <motion.div
      ...
      // Fondo transparente para ver la lava
      className={`group relative overflow-hidden border-b border-white/10 bg-transparent transition-all duration-700 ${
          ...
          >
            <div className="px-8 pb-10 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 items-end">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {technologies.map((tech, i) => (
                      <motion.span 
                        ...
                        // 🔥 CAMBIO: Etiquetas en Orange-400 y borde sutil
                ...
                >
                  {link && (
                    // 🔥 CAMBIO: Hover naranja en el botón de Demo
                    <a href={link} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-full bg-white px-6 py-3 text-xs font-bold text-black hover:bg-orange-500 transition-colors">
                      <ExternalLink size={14} /> Demo
                    </a>
                  )}
                  {github && (
                    <a href={github} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-full border border-white/20 bg-black/40 px-6 py-3 text-xs font-bold text-white hover:bg-white/10 transition-colors">
